...
def _merge_node_layer(edges_df: pd.DataFrame, gff_prokka: pd.DataFrame):
    ...
    df = pd.concat([one_merged,two_merged,edges_df.weight,edge_length, edge_scaf, edge_annot],axis=1)
...
def build_multilayer_network(cdhit_edges: pd.DataFrame, prokka_edges: pd.DataFrame, bin_edges: pd.DataFrame, gff_prokka: pd.DataFrame) -> JSONDataSet:
    ...
    #cdhit_edges2 = _merge_node_layer(cdhit_edges, gff_prokka)
    ...
    G = nx.from_pandas_edgelist(prokka_edges2,'n1','n2',['weight','edge_length','edge_scaf','edge_annot'])
    print(G.size())
    print(G.order())
    pdb.set_trace()
    sparql = SPARQLWrapper('http://rdf.geneontology.org/blazegraph/sparql')#../../../../data/01_raw/sparql')
    ...
    res = [G.subgraph(x) for x in sorted(nx.connected_components(G),key=len,reverse=True)]
